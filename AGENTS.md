...
<dual_operator_contract>
## Harness-Kit Dual-Operator Contract
This project runs on a token-ring locked dual-agent system mapping Antigravity (IDE) and Codex (Terminal).
1. Codex Agent runs the workflow using `npm run harness:exec` for safe bash interactions.
2. IDE Agent (Antigravity) acts as the code architect.
3. The `harness task` API IS THE SINGLE SOURCE OF TRUTH for task state.
4. You MUST run `npm run harness:dream` at the end of large sessions to compact context memory.
5. Use `npm run harness:spawn -- --agent <role> --task "..."` to delegate sub-tasks to native specialists.
</dual_operator_contract>
